...
if (isset($_POST ["op"]))
{
	$op = $_POST ["op"];
	if ($op=="veiculo")
		$consulta=mysqli_query($bd,"select * from veiculos where veiculo like '%$expressao%'");
	if ($op=="marca")
		$consulta=mysqli_query($bd,"select * from veiculos where marca like '%$expressao%'");
	if ($op=="nome")
...
while ($reg!=0)
{
	...
	?>
	<a href="regravar.php?pl=<?php echo $placa;?>" onclick = "return confirm ('Exclui o registro?');">Excluir</a>
